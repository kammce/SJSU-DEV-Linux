...
inline bool wireless_is_ack_required(mesh_packet_t *pkt) {
    return mesh_is_ack_required(pkt);
}

/// Just a wrapper around mesh_send() to put all wireless related API at this file.
inline bool wireless_send(uint8_t dst_addr, mesh_protocol_t protocol, const char *data, int len, uint8_t max_hops) {
...
inline bool wireless_send_formed_pkt(mesh_packet_t *pkt) {
