...
{
    xSemaphoreGiveFromISR(m_nrf_activity_sem, NULL);
}
...
static int nrf_driver_init(void* p, int len)
{
    ...
    eint3_enable(BIO_NORDIC_IRQ_P0PIN, eint_falling_edge, nrf_irq_callback);
...
static int nrf_driver_send(void* p, int len)
{
	 ...
	 * restricted to be called from a task alone.
	 */
	if (taskSCHEDULER_RUNNING == xTaskGetSchedulerState()) {
	    xSemaphoreGiveFromISR(m_nrf_activity_sem, NULL);
	}
...
{
    const int ok = (sizeof(uint32_t) == len);
