...
bool tlm_variable_set_value(const char *comp_name, const char *name, const char *value)
{
    ...
    switch(reg_var->elm_type) {
        ...
        case tlm_float:
        {
            ...
            int sig = 0;
            unsigned int dec = 0;
            if(sscanf(value, "%i.%1u", &sig, &dec) >= 1) {
                float num = dec;
                num /= 10;
                num += sig;
    ...
    switch(reg_var->elm_type) {
        case tlm_int:
        {
            ...
            if (1 == reg_var->elm_size_bytes) {
                ...
                snprintf(buffer, len, "int32:%i", *data);
                tlm_variable_print_array(",%i", data, buffer);
            }
            else if (8 == reg_var->elm_size_bytes) {
                int64_t *data = (int32_t*)(reg_var->data_ptr);
                snprintf(buffer, len, "int64:%li", *data);
                tlm_variable_print_array(",%li", data, buffer);
            }
            else {
                success = false;
            }
        ...
        case tlm_uint:
        {
            ...
            if (1 == reg_var->elm_size_bytes) {
                ...
                snprintf(buffer, len, "uint32:%u", *data);
                tlm_variable_print_array(",%u", data, buffer);
            }
            else if (8 == reg_var->elm_size_bytes) {
                uint64_t *data = (uint32_t*)(reg_var->data_ptr);
                snprintf(buffer, len, "uint64:%lu", *data);
                tlm_variable_print_array(",%lu", data, buffer);
            }
            else {
                success = false;
            }
        ...
        case tlm_bit_or_bool:
        {
            ...
            for (uint16_t i=1; i < reg_var->elm_arr_size; i++) {
                ++data;
                size_t curr_len = strlen(buffer);
