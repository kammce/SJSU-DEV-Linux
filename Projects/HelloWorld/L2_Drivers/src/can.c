...
typedef struct {
    void_func_t bus_error;      ///< When serious BUS error occurs
    void_func_t data_overrun;   ///< When we read the CAN buffer too late for incoming message
} can_callbacks_t;
...
static void CAN_handle_isr(const uint32_t ibits, const can_t can, volatile LPC_CAN_TypeDef *CANx)
{
    ...
    if (ibits & intr_rx) {
        /**
         * TODO Test if we directly have FreeRTOS queue copy the data from CAN registers.
...
bool CAN_init(can_t can, uint32_t baudrate_kbps, uint16_t rxq_size, uint16_t txq_size,
              void_func_t bus_off_cb, void_func_t data_ovr_cb)
{
    const uint8_t cidx = CAN_INDEX(can);
     ...
     */
    LPC_SC->PCLKSEL0 &= ~(0x3F << 26);
    LPC_SC->PCLKSEL0 |=  (0x15 << 26);
