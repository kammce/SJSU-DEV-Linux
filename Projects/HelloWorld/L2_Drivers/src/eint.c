...
/// Linked list structure of eints
...
/// Instance of list of eints
eint3_entry_t *m_eint3_list_head = NULL;
    ...
    while (e && status) {
        ...
        if (e->pin_mask & status) {
            ...
            if (e->pin_mask & UINT32_MAX) {
                ...
                LPC_GPIOINT->IO2IntClr = (e->pin_mask >> p2_bit_num);
            }
        }
        e = e->next;
    };
...
void eint3_enable(uint8_t pin_num, eint_intr_t type, void_func_t func)
{
    eint3_entry_t *e = NULL;
    const uint8_t p2_bit_num = 32;

    if (pin_num <= 63 && NULL != func &&
       ...
       )
    {
        ...
        /* PORT0 */
        if (pin_num < p2_bit_num) {
            if (eint_falling_edge == type) LPC_GPIOINT->IO0IntEnF |= e->pin_mask;
            else  LPC_GPIOINT->IO0IntEnR |= e->pin_mask;
        }
        /* PORT2 */
        else {
            if (eint_falling_edge == type) LPC_GPIOINT->IO2IntEnF |= (e->pin_mask >> p2_bit_num);
            else LPC_GPIOINT->IO2IntEnR |= (e->pin_mask >> p2_bit_num);
        }
