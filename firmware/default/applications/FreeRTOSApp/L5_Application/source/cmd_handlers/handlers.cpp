...
CMD_HANDLER_FUNC(suspenderHandler)
{
    char Task[30] = {0};

    uint8_t Validate_Suspend = cmdParams.scanf("suspend %s", Task);
    uint8_t Validate_Resume = cmdParams.scanf("resume %s", Task);

    scheduler_task *vTaskAction = scheduler_task::getTaskPtrByName(Task);
    ...
    if(Validate_Suspend == 1)
    {
        vTaskSuspend( vTaskAction->getTaskHandle() );
    }
    else if(Validate_Resume == 1)
    {
        vTaskResume( vTaskAction->getTaskHandle() );
    }
    else
    {
...
{
    scheduler_task *vOrientationTask = scheduler_task::getTaskPtrByName("vOrientationTask");
    if(cmdParams == "on")
    {
        vTaskResume( vOrientationTask->getTaskHandle() );
    }
    else if(cmdParams == "off")
    {
        vTaskSuspend( vOrientationTask->getTaskHandle() );
    }
    else
    {
        printf("Invalid Arguments\n");
