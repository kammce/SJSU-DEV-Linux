...
char UART2Send(char out)
{
    LPC_UART2->THR = out;
    // Wait for UART2 to finish sending
    while(LPC_UART2->LSR & (1 << 5));
    return out;
}

char UART3Send(char out)
{
    LPC_UART3->THR = out;
    // Wait for UART3 to finish sending
...
CMD_HANDLER_FUNC(UARTLab)
{
    ...
    printf("Printed #%s through UART2 & UART3\n\n", name);

    return true;
}


CMD_HANDLER_FUNC(spiLab)
{
    ...
    //// Manufacture ID
    char ManufactureID      = spi1_ExchangeByte(0x00);
    //// Device ID P1
    char DeviceID1          = spi1_ExchangeByte(0x00);
    //// Device ID P1
    char DeviceID2          = spi1_ExchangeByte(0x00);
    LPC_GPIO0->FIOSET      |= (1 << 6);
    ...
    char SPIFlashStatus     = spi1_ExchangeByte(0x00);
    LPC_GPIO0->FIOSET      |= (1 << 6);
    ...
    //// Start Reading from address 0x0000
                              spi1_ExchangeByte(0x00);
                              spi1_ExchangeByte(0x00);
    char SPIPage[PAGE_SIZE]       = { 0 };

    printf("[ MAIN MEMORY READ MSB 512 Bytes ]");

    for (uint32_t i = 0; i < PAGE_SIZE; ++i)
    {
        SPIPage[i]          = spi1_ExchangeByte(0x00);
        if(i % 16 == 0)
        {
            printf("\n\t0x%04X:: ", i);
        }
        printf("%02X ", SPIPage[i]);
    }
    printf("\n\n");

    LPC_GPIO0->FIOSET      |= (1 << 6);

    const char *SPIStatuses[][2] = {
        /************
           { 0, 1 }
             ^  ^
             |  |
         false  true
        **************/
        { "Page Size set to 264-bytes.",   "Page Size set to 'power of 2' (256 bytes) size." }, // 0th bit
        { "Sector protection is disabled.", "Sector protection is enabled."}, //1st bit
        { "Buffer and main memory page DO NOT match ", "Buffer and main memory page MATCH."}, //6th bit
        { "Device is BUSY", "Device is READY" }, // 7th bit
    };
    ...
    const char InvalidDensity[] = "Invalid ATMEL DataFlash Density Code";
    ...
    uint8_t status = (SPIFlashStatus >> 4) | (SPIFlashStatus & 0b00000011);
    uint8_t density = (SPIFlashStatus >> 2) & 0b00001111;
...
CMD_HANDLER_FUNC(i2cIoHandler)
{
    ...
    if (read) {
        if (cmdParams.scanf("%*s 0x%x 0x%x %u", &addr, &reg, &count) < 2) {
            output.putline("Need device and register address");
            return false;
        }

        output.printf("addr = 0x%X :: reg = 0x%X :: counter = %d", addr, reg, count);
        ...
        if (3 != cmdParams.scanf("%*s 0x%x 0x%x 0x%x", &addr, &reg, &data)) {
